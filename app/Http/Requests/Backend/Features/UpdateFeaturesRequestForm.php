...
class UpdateFeaturesRequestForm extends FormRequest
{
    /**
    ...
    {
        $feature = Feature::where('name_en', $this->name_en)->orWhere('name_ar', $this->name_ar)->where('feature_type_id', $this->feature_type_id)->first();
        if ($feature) {
            return [
                'name_en' => 'required|unique:features,name_en,' . $this->id,
                'name_ar' => 'required|unique:features,name_ar,' . $this->id,
