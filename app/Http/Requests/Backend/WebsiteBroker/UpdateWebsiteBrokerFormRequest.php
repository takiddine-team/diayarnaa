...
class UpdateWebsiteBrokerFormRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        ...
        $website_broker = WebsiteBroker::find($this->id);

        if (isset($website_broker->image)) {
            return [
                'name' => 'required',
                'last_name' => 'required',
                'email' => 'required|email|unique:website_brokers,email,' . $this->id,
                ...
                'phone' => 'required|unique:website_brokers,phone,'. $this->id,
