                ...
                <div>
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb p-0">
                            ...
                            <li class="breadcrumb-item">
                                <i class="fas fa-users-cog"></i> جميع سياسات الخصوصية
                            </li>
                        </ol>
                    </nav>
                </div>
                <div>
                    <a href="{{ route('super_admin.privacy_policy-create') }}" class="mb-1 btn btn-primary"><i
                            class="mdi mdi-playlist-plus"></i>اضافة </a>
                    <a href="{{ route('super_admin.privacy_policy-showSoftDelete') }}" class="mb-1 btn btn-danger"><i
                            ...
                            @if (isset($policies) && $policies->count() > 0)
                                @foreach ($policies as $policy)
                                    <tr>
                                        ...
                                        <td style="text-align: center">{!! isset($policy->privacy_description_en)
                                            ? $policy->privacy_description_en
                                            : "<span style='color:blue;'>----------</span>" !!} </td>
                                        <td style="text-align: center">{!! isset($policy->privacy_description_ar)
                                            ? $policy->privacy_description_ar
                                            : "<span style='color:blue;'>----------</span>" !!} </td>
                                        ...
                                        <td style="text-align: center">
                                            <a href="{{ route('super_admin.privacy_policy-edit', $policy->id) }}"
                                                class="mb-1 btn btn-sm btn-success"><i
                                                    class="mdi mdi-playlist-edit"></i></a>
                                            <a href="{{ route('super_admin.privacy_policy-softDelete', [isset($policy->id) ? $policy->id : '----------']) }}"
                                                class="confirm mb-1 btn btn-sm btn-danger" title="Delete"><i
                                                    class="mdi mdi-delete"></i></a>
                                            <a href="{{ route('super_admin.privacy_policy-activeInactiveSingle', [isset($policy->id) ? $policy->id : '----------']) }}"
