                ...
                <div>
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb p-0">
                            ...
                            <li class="breadcrumb-item">
                                <i class="fas fa-users-cog"></i> جميع طلبات الوظائف
                            </li>
                        </ol>
                    </nav>
                </div>
            ...
            <div class="card card-default">
                ...
                <div class="card-body">
                    <table id="hoverable-data-table" class="table table-hover table-striped">
                        ...
                        <tbody>
                            @if (isset($job_requests) && $job_requests->count() > 0)
                                @foreach ($job_requests as $job)
                                    <tr>
                                        <td style="text-align: center">{!! isset($job->job->title_en) ? $job->job->title_en : "<span style='color:blue;'>----------</span>" !!} </td>
                                        <td style="text-align: center">{!! isset($job->job->title_ar) ? $job->job->title_ar : "<span style='color:blue;'>----------</span>" !!} </td>
                                        <td style="text-align: center">{!! isset($job->first_name) ? $job->first_name : "<span style='color:blue;'>----------</span>" !!} </td>
                                        <td style="text-align: center">{!! isset($job->phone) ? $job->phone : "<span style='color:blue;'>----------</span>" !!} </td>
                                        <td style="text-align: center">
                                            {!! isset($job->created_at) ? $job->created_at : "<span style='color:red;'>Undefined</span>" !!}
                                        </td>
                                        <td style="text-align: center">
                                            <a href="{{ route('super_admin.job_requests-show', isset($job->id) ? $job->id : -1) }}"
                                                class="mb-1 btn btn-sm btn-primary"><i class="mdi mdi-eye"></i></a>

                                            <a href="{{ route('super_admin.job_requests-destroy', [isset($job->id) ? $job->id : '----------']) }}"
